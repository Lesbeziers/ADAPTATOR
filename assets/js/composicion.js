...
const ComposicionMovil = (() => {
  ...
  async function generate() {
    ...
    try {
      ...
      if (typeof Layout !== 'undefined' && Layout.isFreemium('MOVIL TXT') && typeof Pastilla !== 'undefined') {
        const src  = Pastilla.getFreemiumSrc();
...
// ============================================================
// COMPOSICION TEXTO — genérico para TEXTO HORIZONTAL / VERTICAL
// generate(masterFormat, compName, flagKey)
// ============================================================

const ComposicionTexto = (() => {


  const _generating = {};
  const _pending    = {};

  async function generate(MASTER_FORMAT, COMP_NAME, flagKey) {
    if (_generating[MASTER_FORMAT]) { _pending[MASTER_FORMAT] = true; return; }
    if (!State.formatSizes[MASTER_FORMAT]) return;

    _generating[MASTER_FORMAT] = true;
    _pending[MASTER_FORMAT]    = false;
    ...
    try {
      ...
      let comp = State.layers.find(l => l[flagKey]);
      if (!comp) {
        comp = {
          id:            'layer_comp_' + Date.now(),
          name:          COMP_NAME,
      ...
      _generating[MASTER_FORMAT] = false;
      if (_pending[MASTER_FORMAT]) generate(MASTER_FORMAT, COMP_NAME, flagKey);
    }
  }
